...
object ConversationForm {
  def apply() =
    Form[ConversationData](
      mapping(
        "query" -> tuple("subject" -> optional(text).verifying("Subject is required", _.isDefined),
          "message" -> optional(text).verifying("Message is required", _.isDefined),
          "language" -> optional(text)),

        "sender" -> tuple("name" -> optional(text).verifying("Sender name is required", _.isDefined),
        ...
        "customer" -> tuple("name" -> optional(text),
          "email" -> optional(text),
  ...
  case class ConversationData(query: (Option[String], Option[String],  Option[String]),
